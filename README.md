...
## 制作背景
バックエンドの処理フローを構造的に理解することを目的に、Spring Bootを用いてRESTAPIの設計から実装までを一貫して行いました。
Controller でリクエストを受け取り、Service で データの取得・登録・更新や、登録時の重複チェックを行い、Mapperでデータベースにアクセスする構造を採用しています。
これにより、データの取得や追加・更新処理をフローに沿って整理し、標準的な MVCモデルに則った実装が可能となっています。
